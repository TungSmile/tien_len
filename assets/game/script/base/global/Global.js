...
cc.Global.loginType = LoginType.FastLogin;///LoginType.Normal;
...
cc.Global.getAllCountTimeStringByMillis = function (millis) {
    if (millis <= 0) return "00:00";
    var lsecs = Math.round(millis / 1000).toFixed(0);
    // var hour = (lsecs / 3600).toFixed(0);
    var hour = Math.floor(lsecs / 3600);
...
cc.Global.blackHole = cc.color('#000000');
...
cc.Global.instanceAlert = function (cb) {
    cc.resources.load("prefabs/Alert", cc.Prefab, function (completedCount, totalCount, item) {
    }, function (err, prefab) {
        if (!err) {
            cc.Global.alertPrefab = prefab;
            if (cb) {
                cb(null, prefab);
            }
        } else {
            cc.log("k load dc prefab alert");
            if (cb) {
                cb(true, null);
            }
        }
...
cc.Global.instanceLoading = function (cb) {
    if (!cc.Global.loadingPrefab || (cc.Global.loadingPrefab && !cc.isValid(cc.Global.loadingPrefab))) {
        cc.resources.load("prefabs/LoadingDialog", cc.Prefab, function (completedCount, totalCount, item) {
        }, function (err, prefab) {
            if (!err) {
        ...
        if (cb) {
            cb(null, cc.Global.loadingPrefab);
        }
    }
};

cc.Global.showLoading = function (data) {
    if (cc.Global.loadingPrefab && cc.isValid(cc.Global.loadingPrefab)) {
        var dialog = cc.instantiate(cc.Global.loadingPrefab);
        dialog.name = "LoadingDialog";
        ...
        if (canvas) {
            ...
            if (data) {
                if (data.parrent) {
                    data.parrent.addChild(dialog, cc.macro.MAX_ZINDEX);
                } else {
                    canvas.addChild(dialog, cc.macro.MAX_ZINDEX);
                }
                if (data.button) {
                    dialog.addChild(data.button);
                }
                if (!data.idle) {
                    cc.Global._timeOutHideLoading = cc.Global.timeOutHideLoading();
                }
            } else {
                canvas.addChild(dialog, cc.macro.MAX_ZINDEX);
                cc.Global._timeOutHideLoading = cc.Global.timeOutHideLoading();
...
cc.Global.timeOutHideLoading = function () {
    return setTimeout(function () {
        cc.Global.hideLoading();
    }, 10000)
};
cc.Global.hideLoading = function () {
    if (cc.Global._timeOutHideLoading) {
        clearTimeout(cc.Global._timeOutHideLoading);
    }
    ...
    for (let i = 0; i < nodes.length; i++) {
        if (nodes[i].name == "LoadingDialog") {
            listDestroy.push(nodes[i]);
        }
    }
    for (let j = 0; j < listDestroy.length; j++) {
        var node = listDestroy[j];
        node.destroy();
...
cc.Global.initSystemMessage = function (cb) {
    if (!cc.Global.systemMessagePrefab) {
        // prefabs/SystemMessage
        cc.resources.load("prefabs/SystemMessage", cc.Prefab, function (completedCount, totalCount, item) {
        }, function (err, prefab) {
            if (!err) {
        ...
        if (cb) {
            cb(null, cc.Global.systemMessagePrefab);
        }
    }
}
...
cc.Global.showMessage = function (message) {
    if (cc.Global.systemMessagePrefab) {
        ...
        if (cc.Global.loadingPrefab && cc.isValid(cc.Global.loadingPrefab)) {
            ...
            cc.Global.instanceLoading(function (err, prefab) {
                if (!err) {
                    cc.Global.initSystemMessage(function (err2, prefab2) {
                        if (!err2) {
                            var popup = cc.instantiate(cc.Global.systemMessagePrefab);
...
cc.Global.loadClip = function () {
    cc.resources.loadDir("anim/emoji", cc.AnimationClip, (error, clipArr, urlArr) => {
        if (!error) {
...
cc.Global.firstTimeLogin = true;
cc.Global.firstTimeActiveSuKien = true;