...
var tlmnLogic = {
    ...
    lstCardFind(lstDanh, lstCard) {
        var lst = [];
        lstCard.forEach(value => {
            if (lstDanh.indexOf(value) < 0) {
                lst.push(value);
            }
        });
        return lst;
    ...
    suggest(lst, lastCard, value) {
        ...
        if (lastCard.length == 0) {
            ...
            if (parseInt(lastCard[0] / 4) == 15 && lastCard.length < 3) {
                ...
                findDoiThong.forEach(item => {
                    if (item.indexOf(value) > -1 && (item.length > 6 || lastCard.length == 1)) {
                        lstSuggest = item;
                    }
                });
                if (lstSuggest.length == 0 && lastCard.length == 1 && value > lastCard[0]) {
                    lstSuggest = [value];
                }
            } else if (type == 3) {
    ...
    parse_1104_message(message) {
        ...
        return this.SapXep(lastCard);
    },
    parse_1104_message2(lstNode) {
        var lst = [];
        lstNode.forEach(node => {
            var cm = node.getComponent("PhomCard");
    ...
    checkTurnCard(lstSuggest, children) {
        children.forEach(node => {
            var serverValue = Number(node.getComponent("PhomCard").serverValue);
            if (lstSuggest.indexOf(serverValue) < 0) {
                node.getChildByName("background").getChildByName('bg_black').active = true;
            } else {
                node.getChildByName("background").getChildByName('bg_black').active = false;
            }
        })
    }
