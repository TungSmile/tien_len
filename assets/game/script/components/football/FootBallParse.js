...
var FootBallParse = {
    parse(message) {
        ...
        switch (Number(tempData.messageId)) {
            ...
            case 1104: {
                ...
                if (Linker.ZONE == FootBallConstant.ZONE_ID.ZONEID_PENALTY) {
                    return this.TurnKickBallResponse(tempData.messageId, tempData.status, tempData.data);
                }
            }
            case 1242: {
    ...
    parse_1108_message(messageId, status, data) {
        ...
        if (status == 1) {
            ...
            //data: "0242991{"x":18.899,"y":0.204,"z":0.0}24299102429920"
            var tempData = data;
            var tempArray = tempData.split(SocketConstant.SEPERATOR.ELEMENT);
            message.matchStartID = tempArray[0];

            message.turnPlayer = tempArray[1];
            tempArray[2] = tempArray[2].split(SocketConstant.SEPERATOR.DIFF_ARRAY);
            message.shootingPosition = JSON.parse(tempArray[2][0]);
            tempArray[3] = tempArray[3].split(SocketConstant.SEPERATOR.DIFF_ARRAY);
            message.timeLeft = tempArray[3][0];
            tempArray[4] = tempArray[4].split(SocketConstant.SEPERATOR.ARRAY);

            message.listPlayer = [];
            message.listPlayer.push({
                id: tempArray[3][1],
                isBot: tempArray[4][0]
            });
    ...
    GameEndResponse(messageId, status, data) {
        cc.log("FootBall EndGameResponse ", data);
        var message = {
            messageId: messageId,
            status: status
        };
        if (status == 1) {
            ...
            //normal win bida
            message.winType = FootBallConstant.TYPE_WIN.NORMAL_WIN;
            ...
            for (var i = 0; i < roomParam.length; i++) {
                ...
                player.level = playerResult[8];
                player.levelUpMoney = playerResult[9];
    ...
    ReconnectionResponse(messageId, status, data) {
        ...
        if (status == 1) {
            ...
            message.posStart = JSON.parse(gameInfo[7]);
            message.timeLeft = gameInfo[8];
            ...
            for (var i = 0; i < playerStr.length; i++) {
                ...
                player.totalScore = p[9];
                player.countryId = p[7];
                player.countAutoPlay = p[10];
    ...
    TurnCardResponse1vs4: function (messageId, status, data) {
        ...
        if (message.status == 1) {
            ...
            switch (message.cueStatus) {
                case FootBallConstant.TABLE_STATUS.MOVING_CUE:
                    break;
                case FootBallConstant.TABLE_STATUS.UPDATE_GOC_BAN_BI:
                    break;
                case FootBallConstant.TABLE_STATUS.END_DRAGGING_CUE:
                    break;
                case FootBallConstant.TABLE_STATUS.SETTING_RESULT_TURN:
                    var tempArray2 = tempArray[1];
                    ...
                    break;
                case FootBallConstant.TABLE_STATUS.AUTO_NEXT_TURN:
        ...
        if (status == 1) {
            ...
            switch (message.cueStatus) {
                ...
                case FootBallConstant.TABLE_STATUS.AUTO_NEXT_TURN:
                    ...
                    message.leftTime = Number(cuepr[5]);
                    break;
                case FootBallConstant.TABLE_STATUS.RECOVER_BALL_POSITION_RECONNECT:
    ...
    getPlayersSetting: function (result) {
        ...
        for (var i = 0; i < result.length; i++) {
            ...
            if (Linker.ZONE == FootBallConstant.ZONE_ID.ZONEID_BIDA_PHOM) {
    ...
    TurnKickBallResponse: function(messageId, status, datas) {
        // "3242991{"x":18.899,"y":0.204,"z":-6.0}0true{"x":0.0,"y":0.0,"z":0.0}24299232429912"
        if (status == 1)
        {
            var tempData = datas;
            var tempArray = datas.split(SocketConstant.SEPERATOR.ELEMENT);

            var message = {
                messageId: messageId,
                status: status
            };
            message.cueStatus = tempArray[0];
            message.turnPlayer = tempArray[1];
            message.posKick = JSON.parse(tempArray[2]);
            message.idAnim = tempArray[3];
            message.hitGoal = tempArray[4];
            message.listPlayer = [];
            tempArray[6] = tempArray[6].split(SocketConstant.SEPERATOR.DIFF_ARRAY);
            message.posEnd = JSON.parse(tempArray[5]);
            message.timeLeft = tempArray[6][0];
            message.listPlayer.push({
                id: tempArray[6][1],
                totalScore: tempArray[7]
            });
            message.listPlayer.push({
                id: tempArray[8],
                totalScore: tempArray[9]
            });
