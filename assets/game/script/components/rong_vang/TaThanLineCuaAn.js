...
cc.Class({
    ...
    activeLine: function (lineIndex, listColumn, lineWin, callback) {
        ...
        if (node) {
            ...
            node.runAction(cc.sequence(cc.delayTime(1.2), cc.callFunc(function () {
                Linker.TaThanLineCuaAn.stopEffectItemWinLine(lineIndex, listColumn);
                node.stopAllActions();
                node.active = false;
                if (callback) {
                    callback();
            ...
            Linker.TaThanLineCuaAn.playEffectItemWinLine(lineIndex, listColumn);
        }
    ...
    playEffectItemWinLine(lineIndex, listColumn) {
        var temp = Linker.TaThanLineCuaAn.WIN[lineIndex];
        //cc.log(temp);
