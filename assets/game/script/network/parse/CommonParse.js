...
var CommonParse = {
    parse(message) {
        ...
        switch (Number(tempData.messageId)) {
            ...
            case 12015: {
                return this.parse_12015_message(tempData.messageId, tempData.status, tempData.data);
            }
            case 1203: {
            ...
            case 12031: {
                return this.parse_12031_message(tempData.messageId, tempData.status, tempData.data);
            }
            case 19721: {
                return this.parse_19721_message(tempData.messageId, tempData.status, tempData.data);
            }
        }
    },
    ...
    parse_12015_message(messageId, status, data) {
        cc.error(data);
        var message = {
            messageId: messageId,
            status: Number(status),
        };
        //status = 1 la de nhan tien
        if (message.status == 1) {
            message.data = JSON.parse(data);
        }
        //status = 0 la check xem nguoi choi da nhan nhung muc nao
        else if (message.status == 0 && data.length > 0)
        {
            message.data = JSON.parse(data);
        }
        else
    ...
    parse_100001_message(messageId, status, data) {
        ...
        if (status == 1) {
            ...
            message.userExp = tempArray[3];
            message.checkMail = tempArray[4];
            message.userLevel = tempArray[5];
            // message.isPayment = tempArray[6];
            message.alertEmailContent = tempArray[6]; //used cho country
            if (message.alertEmailContent == "") {
                message.alertEmailContent = "null";
            }
                ...
                tempArray1.forEach((element1, pos1) => {
                    ...
                    if (pos1 == 0) {
                        message.rooms.push({
                            ...
                            canAccess: tempArray2[3] === "false" ? false : true,
                            levelAccess: tempArray2[4],
                            online: tempArray2[5],
                            tables: [{
                                tableIndex: -1,
                                tableSize: 0,
    ...
    parse_12031_message(messageId, status, data) {
        var message = {
            messageId: messageId,
            status: status
        };
        if (status == 1) {
            var tempData = data;
            var tempArray = tempData.split(SocketConstant.SEPERATOR.ELEMENT);
            message.message = Number(tempArray[0]);
            message.minBet = Number(tempArray[1]);
            message.zoneId = Number(tempArray[2]);
            message.money = Number(tempArray[3]);
            message.userId = Number(tempArray[4]);
            message.otherUserId = Number(tempArray[5]);
            message.viewName = tempArray[6];
            message.otherViewName = tempArray[7];
        } else {
            message.error = data;
        }
    ...
    parse_19721_message(messageId, status, data) {
        var message = {
            messageId: messageId,
            status: status
        };
        if (status == 1) {
            var tempData = data;
            var tempArray = tempData.split(SocketConstant.SEPERATOR.ELEMENT);
            message.action = Number(tempArray[0]);
            message.bought = Boolean(tempArray[1]);
            message.used = Boolean(tempArray[2]);
            message.realMoney = Number(tempArray[3]);
            message.listBackground = JSON.parse(tempArray[4]);
        } else {
            message.error = Number(data);
        }
