...
var LobbyParse = {
            ...
            case 1100:
                {
                    if (Linker.ZONE == soccerConstant.ZONE_ID.ZONE_1VS1) {
                        return this.parse_1100_soccer_message(tempData.messageId, tempData.status, tempData.data);
                    } else {
    ...
    parse_12021_message(messageId, status, data) {
        var message = {
            messageId: messageId,
            status: status,
            content: data
        ...
        if (status == 1) {
            ...
            message.type = Number(tempArray[0]);
        } else {
            message.error = data;
        }
        return message;
    ...
    parse_1100_soccer_message(messageId, status, data) {
        var message = {
            messageId: messageId,
            status: status
        };
        if (status == 1) {
            ...
            player.isMaster = 1;
            player.countryId = Linker.userData.countryId;
            player.styleId = tempArray[6];
            player.utype = tempArray[5];
            player.playerPosition = tempArray[7];
            player.formationPlayer = tempArray[8];
            player.score = "";
